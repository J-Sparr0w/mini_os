...
/// For Foreground and background colors packed into a u8
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(transparent)]
struct ColorCode(u8);
...
impl Writer {
    pub fn write_byte(&mut self, byte: u8) {
        match byte {
            ...
            byte => {
                ...
                self.vga_buffer.chars[row][col].write(VgaChar {
                    ascii_char: byte,
                    color_code,
                });
    ...
    fn new_line(&mut self) {
        for i in 1..BUFFER_HEIGHT {
            for j in 0..BUFFER_WIDTH {
                self.vga_buffer.chars[i - 1][j].write(self.vga_buffer.chars[i][j].read());
            }
        }
    ...
    fn clear_row(&mut self, row: usize) {
        for i in 0..BUFFER_WIDTH {
            self.vga_buffer.chars[row][i].write(VgaChar {
                ascii_char: b' ',
